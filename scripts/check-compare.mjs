...
const words = (t) => (String(t).replace(/```[\s\S]*?```/g, ' ').replace(/<[^>]+>/g, ' ').replace(/\[([^\]]*)\]\([^)]*\)/g, '$1').replace(/[#>*_`|-]/g, ' ').match(/[\p{L}\p{N}$%'.,]+/gu) || []).length;
...
for (const id of only.length ? only : files) {
  ...
  const n = words(body);
  if (n < 3000 || n > 3600) bad.push(`объём ${n} слов (нужно 3000-3500)`);
  if (fm.title?.length < 45 || fm.title?.length > 62) bad.push(`title ${fm.title?.length} знаков`);
  ...
  for (const f of fm.faq || []) { const w = words(f.answer); if (w < 35 || w > 100) warn.push(`ответ ${w} слов: «${f.question}»`); }
  ...
  for (const l of links) if (!l.endsWith('/') && !l.includes('#')) warn.push(`ссылка без слэша: ${l}`);
