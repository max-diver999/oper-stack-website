...
const TIER_BY_PRODUCT: Record<string, ReportTier> = {
  // Сняты с витрин 13 сентября 2026. Whop пересоздаёт товары, когда чинит что-то у себя:
  // публичный адрес остаётся тот же, а идентификатор меняется, и старый просто перестаёт
  // существовать. Прошлые два (prod_xGGOfxv4ZJ9bf и prod_8YWVnEpYSnOSe) именно так и умерли,
  // и покупка узнавалась только по названию товара. Проверять их стоит при каждой правке
...
export type ReportJob = {
  ...
  score?: number;
  /**
   * Результат проверки со страницы целиком. Отчёт берёт его как есть и второй раз не мерит:
   * два честных замера одного живого сайта расходятся на пару баллов, и покупатель увидел бы
   * на странице одно число, а в письме другое.
   */
  visibility?: unknown;
};
