...
async function record(result: any, request: Request, from: { source?: string; campaign?: string }): Promise<void> {
  ...
  await logCheck({ agent: request.headers.get('user-agent') ?? '', areas: (result.areas ?? []).map((a: any) => a.score ?? '—').join('/'), lang: 'en', host: result.host, score: result.score, grade: result.grade, source, campaign, page });
}
